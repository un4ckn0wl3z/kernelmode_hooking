...
PVOID get_system_module_base(const char* module_name) 
{
	...
	PRTL_PROCESS_MODULES modules = (PRTL_PROCESS_MODULES)ExAllocatePoolWithTag(NonPagedPool, bytes, 0x756E6B34); // unk4
	...
	// same with taking snapshot in usermode
	...
	if (module_base <= NULL) return NULL;
...
ULONG64 get_module_base_x64(PEPROCESS proc, UNICODE_STRING module_name)
{
	...
	KeStackAttachProcess(proc, &stat);
	...
	for (PLIST_ENTRY list = (PLIST_ENTRY)pLdr->ModuleListLoadOrder.Flink; list != &pLdr->ModuleListLoadOrder; list = (PLIST_ENTRY)list->Flink) 
	{
		...
		{
			ULONG64 baseAddr = (ULONG)pEntry->DllBase;
			KeUnstackDetachProcess(&stat);
			return baseAddr;
...
bool write_kernel_memory(HANDLE pid, uintptr_t address, void* buffer, SIZE_T size)
{
	...
	KAPC_STATE stat;
	KeStackAttachProcess((PEPROCESS)process, &stat);
